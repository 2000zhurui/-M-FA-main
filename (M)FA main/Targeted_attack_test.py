...
def main(args):
    ...
    #     scene_dataset(root_dir=adv_root_dir,pathfile='H:/code/UAE-RS-main/dataset1/'+DataName+'_test.txt', transform=composed_transforms, mode='adv'),
    #     batch_size=args.val_batch_size, shuffle=False, num_workers=args.num_workers, pin_memory=True)
    #
    # clean_loader = data.DataLoader(
    #     scene_dataset(root_dir=args.root_dir,pathfile='H:/code/UAE-RS-main/dataset1/'+DataName+'_test.txt', transform=composed_transforms),
    #     batch_size=args.val_batch_size, shuffle=False, num_workers=args.num_workers, pin_memory=True)
    adv_loader = data.DataLoader(
        scene_dataset(root_dir=adv_root_dir, pathfile='./dataset1/' + DataName + '_test.txt',
                      transform=composed_transforms, mode='adv'),
        batch_size=args.val_batch_size, shuffle=False, num_workers=args.num_workers, pin_memory=True)

    clean_loader = data.DataLoader(
        scene_dataset(root_dir=args.root_dir, pathfile='./dataset1/' + DataName + '_test.txt',
    ...
    OA_clean, _, _ = test_targeted_acc(target_model, classname, clean_loader, 1, num_classes, target_label=target_label,
                                       print_per_batches=10)
    OA_adv, _, OA_target_adv = test_targeted_acc(target_model, classname, adv_loader, 1, num_classes,
                                                 target_label=target_label, print_per_batches=10)


    print('Clean Test Set OA:', OA_clean * 100)
    print(args.attack_func + ' Test Set OA:', OA_target_adv * 100)


if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('--dataID', type=int, default=1)
    #    parser.add_argument('--root_dir', type=str, default='/code/Data/', help='dataset path.')
    ...
    parser.add_argument('--target_model', type=str, default='alexnet',
                        help='alexnet,vgg16,inception,resnet18,resnet101,densenet121,densenet201,regnet_x_400mf,regnet_x_16gf')
    ...
    parser.add_argument('--attack_func', type=str, default='mixforest',
                        help='fgsm,pgd,cw,ifgsm'
                             'mixforest,mixagricultural,mixairplane,mixbaseballdiamond,mixbeach'
                             'mixairport,mixparking,mixschool,mixstadium')
