...
def test_targeted_acc(model, classname, data_loader, epoch, num_classes, target_label=None, print_per_batches=10):
    model.eval()
    ...
    class_correct = list(0. for i in range(num_classes))
    class_total = list(0. for i in range(num_classes))
    targeted_class_correct = list(0. for i in range(num_classes))
    targeted_class_total = list(0. for i in range(num_classes))
    total = 0
    correct = 0
    class_acc = np.zeros((num_classes, 1))
    targeted_correct = 0
    targeted_class_acc = np.zeros((num_classes, 1))
    ...
    for batch_idx, data in enumerate(data_loader):
        ...
        c = (predicted == labels).squeeze()
        d = (predicted == target_label).squeeze()
        total += batch_size
        correct += (predicted == labels).sum().item()
        targeted_correct += (predicted == target_label).sum().item()
        ...
        for i in range(batch_size):
            ...
            class_correct[label] += c[i].item()
            class_total[label] += 1
            targeted_class_correct[label] += d[i].item()
            targeted_class_total[label] += 1

        if (batch_idx + 1) % print_per_batches == 0:
            print('Epoch[%d]-Validation-[%d/%d] Batch OA: %.2f %%' % (
                epoch, batch_idx + 1, num_batches, 100.0 * (predicted == labels).sum().item() / batch_size))
    ...
    for i in range(num_classes):
        targeted_class_acc[i] = 1.0 * targeted_class_correct[i] / class_total[i]
        print('---------------Accuracy of %12s : %.2f %%---------------' % (
            class_name_list[i], 100 * targeted_class_acc[i]))
    acc = 1.0 * correct / total
    targeted_acc = 1.0 * targeted_correct / total

    print('---------------Epoch[%d]Validation-OA: %.2f %%---------------' % (epoch, 100.0 * acc))
    print('---------------Epoch[%d]Validation-AA: %.2f %%---------------' % (epoch, 100.0 * np.mean(class_acc)))

    if target_label is not None:
        print('---------------Epoch[%d]Validation-Targeted OA: %.2f %%---------------' % (
            epoch, 100.0 * targeted_acc))
        return acc, class_acc, targeted_acc
    else:
        return acc, class_acc
